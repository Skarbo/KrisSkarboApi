...
(function($) {
	...
	function startHandler(event) {
		...
		// If object also has click handler, store it and unbind. Taphold will
		// trigger the
		// click itself, rather than normal propagation.
		if (typeof $elem.data("events") != "undefined" && typeof $elem.data("events").click != "undefined") {
		...
		// Otherwise, if a custom click handler was explicitly defined, then
		// store it instead.
		else if (typeof event.data != "undefined" && event.data != null && typeof event.data.clickHandler == "function") {
		...
		$elem.data("taphold_timer", setTimeout(function() {
			// If event hasn't been cancelled/clicked already, then go ahead and
			// trigger the hold.
			if (!$elem.data("taphold_cancelled") && !$elem.data("taphold_clicked")) {
				// Trigger the hold event, and set the flag to say it's been
				// triggered.
	...
	function stopHandler(event) {
		...
		// Clear the hold timer. If it hasn't already triggered, then it's too
		// late anyway.
		clearTimeout($elem.data("taphold_timer"));

		// If hold wasn't triggered and not already clicked, then was a click
		// event.
		if (!$elem.data("taphold_triggered") && !$elem.data("taphold_clicked")) {
	...
	var taphold = $.event.special.taphold = {
		setup : function(data) {
			...
			if ('ontouchstart' in document.documentElement) {
				$(this).bind("touchstart", data, startHandler).bind("touchend", stopHandler).bind("touchmove", leaveHandler);
			}
			// Mouse
			else {
				$(this).bind("mousedown", data, startHandler).bind("mouseup", stopHandler).bind("mouseleave", leaveHandler);
			}
		},
		teardown : function(namespaces) {
			$(this).unbind("touchstart mousedown", startHandler).unbind("touchend mouseup", stopHandler).unbind("touchmove mouseleave", leaveHandler);
		}
	};
})(jQuery);
