...
class CSSmin {
    ...
    public function run( $css = '', $linebreak_pos = FALSE ) {
        ...
        while ( ( $start_index = $this->index_of( $css, '/*', $start_index ) ) >= 0 ) {
            ...
            $css = $this->str_slice( $css, 0, $start_index + 2 ) . $comment_preserve_string . $this->str_slice( $css,
                    $end_index );
    ...
    private function do_raise_php_limits() {
        $php_limits = array ( 'memory_limit' => $this->memory_limit, 'max_execution_time' => $this->max_execution_time,
    ...
    private function minify( $css, $linebreak_pos ) {
        ...
        for ( $i = 0, $max = count( $this->comments ); $i < $max; $i++ ) {
            ...
            if ( substr( $token, ( strlen( $token ) - 1 ), 1 ) === '\\' ) {
                ...
                $css = preg_replace( $placeholder, self::TOKEN . ( count( $this->preserved_tokens ) - 1 ) . '___',
                        $css, 1 );
                $i = $i + 1; // attn: advancing the loop
                $this->preserved_tokens[] = '';
                $css = preg_replace( '/' . self::COMMENT . $i . '___/',
                        self::TOKEN . ( count( $this->preserved_tokens ) - 1 ) . '___', $css, 1 );
                continue;
            }
        ...
        $css = preg_replace( '/((?<!\\\\)\:|\s)\-?0(?:em|ex|ch|rem|vw|vh|vm|vmin|cm|mm|in|px|pt|pc|%)/iS',
                '${1}0', $css );
        ...
        $css = preg_replace( '/(background\-position|(?:webkit|moz|o|ms|)\-?transform\-origin)\:0(;|\})/ieS',
                "strtolower('$1:0 0$2')", $css );

        // Shorten colors from rgb(51,102,153) to #336699, rgb(100%,0%,0%) to #ff0000 (sRGB color space)
        // Shorten colors from hsl(0, 100%, 50%) to #ff0000 (sRGB color space)
        // This makes it more likely that it'll get further compressed in the next step.
        $css = preg_replace_callback( '/rgb\s*\(\s*([0-9,\s\-\.\%]+)\s*\)(.{1})/i',
                array ( $this, 'rgb_to_hex' ), $css );
        $css = preg_replace_callback( '/hsl\s*\(\s*([0-9,\s\-\.\%]+)\s*\)(.{1})/i', array ( $this, 'hsl_to_hex' ),
                $css );
        ...
        $css = preg_replace( '/(border\-?(?:top|right|bottom|left|)|outline)\:none(;|\})/ieS',
                "strtolower('$1:0$2')", $css );

        // shorter opacity IE filter
        $css = preg_replace( '/progid\:DXImageTransform\.Microsoft\.Alpha\(Opacity\=/i', 'alpha(opacity=',
                $css );
    ...
    private function compress_hex_colors( $css ) {
        ...
        $short_safe = array ( '#808080' => 'gray', '#008000' => 'green', '#800000' => 'maroon',
                '#000080' => 'navy', '#808000' => 'olive', '#800080' => 'purple', '#c0c0c0' => 'silver',
                '#008080' => 'teal', '#f00' => 'red' );
        ...
        while ( preg_match( $pattern, $css, $m, 0, $offset ) ) {
            ...
            if ( $is_filter ) {
                ...
                if ( strtolower( $m[ 2 ] ) == strtolower( $m[ 3 ] ) && strtolower( $m[ 4 ] ) == strtolower( $m[ 5 ] ) && strtolower(
                        $m[ 6 ] ) == strtolower( $m[ 7 ] ) ) {
                    // Compress.
    ...
    private function replace_string( $matches ) {
        ...
        $match = preg_replace( '/progid\:DXImageTransform\.Microsoft\.Alpha\(Opacity\=/i', 'alpha(opacity=',
                $match );
...
class CssminUtil extends CSSmin {

    public static function minifyCss( $css, $linebreakPos = false ) {
        $cssMin = new CSSmin();
        return $cssMin->run( $css, $linebreakPos );
    }
