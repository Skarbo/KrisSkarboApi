...
class InitializeUtil {
    ...
    public static function getClassPathFile( $class_name, $root ) {
        ...
        $path = self::$SEPERATOR . self::getParsedClassNameFolder( $class_name, self::$SEPERATOR );
    ...
    private static function getParsedClassNameFolder( $class_name, $seperator ) {
        ...
        // Preg match all
        $result = preg_match_all( "/((?<=[a-z])[A-Z]|[A-Z](?=[a-z]))/", $class_name, $match );
        
        // Empty result, return class name
        if ( !$result || empty( $match ) || empty( $match[ 0 ] ) || count( $match[ 0 ] ) == 1 ) {
        ...
        foreach ( $match[ 0 ] as $letter ) {
            $text_split = str_replace( $letter, strtolower( "|{$letter}" ), $text_split );
        }
    ...
    private static function getParsedClassNameUnderscore( $class_name ) {
        ...
        // Preg match all
        preg_match_all( "/((?<=[a-z])[A-Z]|[A-Z](?=[a-z]))/", $class_name, $a );
        ...
        foreach ( $a[ 0 ] as $value ) {
            $class_name_return = str_replace( $value, "_" . strtolower( $value ), $class_name_return );
        }
        
        // Removes _ in front of string
        return substr( $class_name_return, 0, 1 ) == "_" ? substr( $class_name_return, 1, 
                strlen( $class_name_return ) ) : $class_name_return;
