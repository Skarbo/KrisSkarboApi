...
class ErrorHandler extends Handler {
    ...
    public function handle( Exception $exception ) {
        ...
        $error_model->setFile( str_replace( $documentRoot, "", $exception->getFile() ) );
        
        // Set line
        $error_model->setLine( $exception->getLine() );
        
        // Set trace
        $error_model->setTrace( 
                str_replace( $documentRoot, "", print_r( $exception->getTraceAsString(), true ) ) );
        ...
        if ( is_a( $exception, AbstractException::class_() ) ) {
            $error_model->setBacktrack( 
                    str_replace( $documentRoot, "", AbstractException::get_( $exception )->getBacktrack() ) );
        }
