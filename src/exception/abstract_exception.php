...
abstract class AbstractException extends Exception {
    ...
    public function __construct( $message = NULL, $custom_code = 0, Exception $previous = NULL ) {
        parent::__construct( $message, $previous && is_int( $previous->getCode() ) ? $previous->getCode() : 0, 
                $previous );
    ...
    private function initBacktrack( array $backtracks ) {
        ...
        for ( $i = 1; $i < count( $backtracks ); $i++ ) {
            ...
            // Class, type, function
            $return[] = Core::cc( "", Core::arrayAt( $backtrack, "class" ), 
                    Core::arrayAt( $backtrack, "type" ), Core::arrayAt( $backtrack, "function" ) );
            
            // File:line
            if ( Core::arrayAt( $backtrack_last, "file" ) && Core::arrayAt( $backtrack_last, "line" ) ) {
                $return[] = Core::cc( ":", Core::arrayAt( $backtrack_last, "file" ), 
