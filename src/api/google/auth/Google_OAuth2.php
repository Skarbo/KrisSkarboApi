...
class Google_OAuth2 extends Google_Auth {
   ...
   */
    public function authenticate( $service, $code = null ) {
        ...
        if ( $code ) {
            ...
            $request = Google_Client::$io->makeRequest( 
                    new Google_HttpRequest( self::OAUTH2_TOKEN_URI, 'POST', array (), 
                            array ( 'code' => $code, 'grant_type' => 'authorization_code', 
            ...
            if ( $request->getResponseHttpCode() == 200 ) {
                ...
                throw new Google_AuthException( "Error fetching OAuth2 access token, message: '$response'", 
   ...
   */
    public function createAuthUrl( $scope ) {
        $params = array ( 'response_type=code', 'redirect_uri=' . urlencode( $this->redirectUri ), 
                'client_id=' . urlencode( $this->clientId ), 'scope=' . urlencode( $scope ), 
                'access_type=' . urlencode( $this->accessType ), 'approval_prompt=' . urlencode( $this->approvalPrompt ) );
   ...
   */
    public function refreshToken( $refreshToken ) {
        $this->refreshTokenRequest( 
                array ( 'client_id' => $this->clientId, 'client_secret' => $this->clientSecret, 
   ...
   */
    public function refreshTokenWithAssertion( $assertionCredentials = null ) {
        ...
        $this->refreshTokenRequest( 
                array ( 'grant_type' => 'assertion', 'assertion_type' => $assertionCredentials->assertionType, 
   ...
   */
    ...
    private function getFederatedSignOnCerts() {
        ...
        $request = Google_Client::$io->makeRequest( 
                new Google_HttpRequest( self::OAUTH2_FEDERATED_SIGNON_CERTS_URL ) );
        if ( $request->getResponseHttpCode() == 200 ) {
        ...
        throw new Google_AuthException( 
                "Failed to retrieve verification certificates: '" . $request->getResponseBody() . "'.", 
                $request->getResponseHttpCode() );
    }
