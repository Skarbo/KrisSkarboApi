...
class SimplehtmldomApi extends simple_html_dom
{

    /**
     * @return file_get_html
     */
    public static function fileGetHtml( $url, $use_include_path = false, $context = null, $offset = -1, $maxLen = -1, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT )
    {
        return file_get_html( $url, $use_include_path, $context, $offset, $maxLen, $lowercase, $forceTagsClosed,
                $target_charset, $stripRN, $defaultBRText );
    }

    /**
     * return str_get_html
     */
    public static function strGetHtml( $str, $lowercase = true, $forceTagsClosed = true, $target_charset = DEFAULT_TARGET_CHARSET, $stripRN = true, $defaultBRText = DEFAULT_BR_TEXT )
    {
...
class simple_html_dom
{
    ...
    protected $self_closing_tags = array ( 'img' => 1, 'br' => 1, 'input' => 1, 'meta' => 1, 'link' => 1, 'hr' => 1,
            'base' => 1, 'embed' => 1, 'spacer' => 1 );
    ...
    protected $optional_closing_tags = array ( 'tr' => array ( 'tr' => 1, 'td' => 1, 'th' => 1 ),
            ...
            'dl' => array ( 'dd' => 1, 'dt' => 1 ), 'p' => array ( 'p' => 1 ), 'nobr' => array ( 'nobr' => 1 ),
            'b' => array ( 'b' => 1 ) );
    ...
    protected function parse_charset()
    {
        ...
        if ( ( strtolower( $charset ) == strtolower( 'ISO-8859-1' ) ) || ( strtolower( $charset ) == strtolower(
                'Latin1' ) ) || ( strtolower( $charset ) == strtolower( 'Latin-1' ) ) )
        {
            if ( is_object( $debugObject ) )
            {
    ...
    protected function parse_attr( $node, $name, &$space )
    {
        ...
        switch ( $this->char )
        {
            case '"' :
                ...
                $this->char = ( ++$this->pos < $this->size ) ? $this->doc[ $this->pos ] : null; // next
                $node->attr[ $name ] = $this->restore_noise(
    ...
    protected function remove_noise( $pattern, $remove_tag = false )
    {
        ...
        for ( $i = $count - 1; $i > -1; --$i )
        {
            ...
            $this->doc = substr_replace( $this->doc, $key, $matches[ $i ][ $idx ][ 1 ],
                    strlen( $matches[ $i ][ $idx ][ 0 ] ) );
        }
